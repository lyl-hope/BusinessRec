...
def processItemSequence(spark, rawSampleDataPath):
    ...
    userSeq = ratingSamples \
        ...
        .groupBy("userId") \
        .agg(sortUdf(F.collect_list("productId"), F.collect_list("timestamp")).alias('movieIds')) \
...
def embeddingLSH(spark, movieEmbMap):
    ...
    movieEmbDF = spark.createDataFrame(movieEmbSeq).toDF("productId", "emb")
...
def generateUserEmb(spark, rawSampleDataPath, model, embLength, embOutputPath, saveToRedis, redisKeyPrefix):
    ...
    ratingSamples = ratingSamples.join(Vectors_df, on='productId', how='inner')
