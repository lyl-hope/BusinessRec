...
object Embedding {
  ...
  def processItemSequence(sparkSession: SparkSession, rawSampleDataPath: String): RDD[Seq[String]] ={
    ...
    val sortUdf: UserDefinedFunction = udf((rows: Seq[Row]) => {
      rows.map { case Row(productId: String, timestamp: String) => (productId, timestamp) }
        .sortBy { case (_, timestamp) => timestamp }
        .map { case (productId, _) => productId }
    })
    ...
    val userSeq = ratingSamples
      ...
      .groupBy("userId")
      .agg(sortUdf(collect_list(struct("productId", "timestamp"))) as "movieIds")
    ...
    ratingSamples.collect().groupBy(_.getAs[String]("userId"))
      .foreach(user => {
        ...
        userEmb = user._2.foldRight[Array[Float]](userEmb)((row, newEmb) => {
          ...
          val movieEmb = word2VecModel.getVectors.get(productId)
          movieCount += 1
  ...
  def trainItem2vec(sparkSession: SparkSession, samples : RDD[Seq[String]], embLength:Int, embOutputFilename:String, saveToRedis:Boolean, redisKeyPrefix:String): Word2VecModel = {
    ...
    val file = new File(embFolderPath.getPath + embOutputFilename)
    val bw = new BufferedWriter(new FileWriter(file))
    for (productId <- model.getVectors.keys) {
      bw.write(productId + ":" + model.getVectors(productId).mkString(" ") + "\n")
    }
    bw.close()
    ...
    if (saveToRedis) {
      ...
      params.ex(60 * 60 * 24)
      for (productId <- model.getVectors.keys) {
        redisClient.set(redisKeyPrefix + ":" + productId, model.getVectors(productId).mkString(" "), params)
      }
      redisClient.close()
    }
  ...
  def embeddingLSH(spark:SparkSession, movieEmbMap:Map[String, Array[Float]]): Unit ={
    ...
    val movieEmbDF = spark.createDataFrame(movieEmbSeq).toDF("productId", "emb")
    ...
    println("productId, emb, bucketId schema:")
    embBucketResult.printSchema()
    println("productId, emb, bucketId data result:")
